<?php 

$page = "alternate-ways";

require __DIR__ . '/../views/header.php';
?>

<!-- ALTERNATE WAYS --> 
<section class="alternate-ways">

    <!-- HEADER -->
    <header class="alternate-ways_header">

        <picture>
            <source srcset="./../graphics/pics/alternate-med-wmc-750.webp" media="(max-width: 750px)">
            <source srcset="./../graphics/pics/alternate-med-wmc-1500.webp" media="(max-width: 1500px)">
            <source srcset="./../graphics/pics/alternate-med.webp" media="(min-width: 1500px)">
            <img src="./../graphics/pics/alternate-med.webp" alt="a chamomile tincture" loading="lazy">
        </picture>
   
        <div class="heading-wrapper">
            <h2 class="animation-element animation--style_clippy-circle animation--duration_500 animation--trigger-point_95 animation--delay_0">Alternative Wege</h2>
        </div>
    </header>

    <!-- TCM --> 
    <section class="alternate-ways_tcm" id="tcm">
        <header class="alternate-ways_tcm_header">
            <!-- <img src="../graphics/pics/tcm-tea.webp" alt="cups of tea" class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000"> -->

            <picture class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000">
                <source srcset="./../graphics/pics/tcm-tea-wmc-750.webp" media="(max-width: 750px)">
                <source srcset="./../graphics/pics/tcm-tea-wmc-1500.webp" media="(max-width: 1500px)">
                <source srcset="./../graphics/pics/tcm-tea.webp" media="(min-width: 1500px)">
                <img src="./../graphics/pics/tcm-tea.webp" alt="cups of tea" loading="lazy">
            </picture>

            <h3  class="animation-element animation--style_fade-up animation--duration_600">TCM</h3>
        </header>

        <!-- ACUPUNCTURE -->
        <article class="alternate-ways_tcm_topic" id="acupuncture">

            <header>  
                <h4 class="animation-element animation--style_flipping-headline animation--delay_300">
                    <span class="heading-markup">Akkupunktur</span>
                </h4>

                <picture class="animation-element animation--style_opacity-in animation--duration_1000">
                    <source srcset="./../graphics/pics/acupuncture-wmc-750.webp" media="(max-width: 750px)">
                    <source srcset="./../graphics/pics/acupuncture-wmc-1500.webp" media="(max-width: 1500px)">
                    <source srcset="./../graphics/pics/acupuncture.webp" media="(min-width: 1500px)">
                    <img src="./../graphics/pics/acupuncture.webp" alt="acupunctur" loading="lazy">
                </picture>
            </header>
            
            <div class="container-description">
                <div class="on-the-point animation-element animation--style_clippy-circle">
                    <p><strong>Lorem ipsum dolor sit, amet consectetur</strong> adipisicing elit. Impedit, esse minus aliquid quaerat molestiae beatae dolores. Eius id consequuntur, nesciunt temporibus, praesentium cum itaque <strong>voluptas saepe</strong> maxime libero architecto voluptatibus.</p>
                </div>
            
                <p class="animation-element animation--style_clippy-circle animation--delay_200">Lorem ipsum dolor sit amet consectetur adipisicing elit. Necessitatibus praesentium, numquam velit corrupti id, accusantium iusto, nulla <strong>repellat cumque voluptas eveniet</strong> sexcepturi assumenda! Totam, explicabo. Debitis nulla quam fugiat eaque?
                Corporis error voluptates ipsa nam impedit eius itaque quaerat quibusdam cumque velit sed repudiandae molestias quis sit hic ipsam, est amet quae assumenda officia?<strong>Perspiciatis rerum est dolore</strong> ratione magni?
                Dolores commodi dolore, doloribus iusto fugiat aut nobis laboriosam similique odit esse sapiente, aliquid neque? Excepturi est voluptatem <strong>exercitationem</strong> ratione impedit in, rem fugit vitae quia quaerat illum laborum itaque.</p>
            </div>
     
           <a href="./alternate-services.php#acupuncture" class="animation-element animation--style_fade-up">Zu meinem Akkupunktur Angebot</a>
        </article>

        <!-- TUINA -->
        <article class="alternate-ways_tcm_topic" id="tuina">

            <header>
                <h4 class="animation-element animation--style_flipping-headline animation--delay_300">
                    <span class="heading-markup">Tuina</span>
                </h4>

                <picture class="animation-element animation--style_opacity-in animation--duration_1000">
                    <source srcset="./../graphics/pics/massage-wmc-750.webp" media="(max-width: 750px)">
                    <source srcset="./../graphics/pics/massage-wmc-1500.webp" media="(max-width: 1500px)">
                    <source srcset="./../graphics/pics/massage.webp" media="(min-width: 1500px)">
                    <img src="./../graphics/pics/massage.webp" alt="massging hands in action" loading="lazy">
                </picture>
            </header>
            
            <div class="container-description">

                <div  class="on-the-point animation-element animation--style_clippy-circle">
                    <p><strong>Lorem ipsum dolor sit, amet consectetur</strong> adipisicing elit. Impedit, esse minus aliquid quaerat molestiae beatae dolores. Eius id consequuntur, nesciunt temporibus, praesentium cum itaque <strong>voluptas saepe</strong> maxime libero architecto voluptatibus.</p>
                </div>

                <p class="animation-element animation--style_clippy-circle animation--delay_200">Lorem ipsum dolor sit amet consectetur adipisicing elit. Necessitatibus praesentium, numquam velit corrupti id, accusantium iusto, nulla <strong>repellat cumque voluptas eveniet</strong> sexcepturi assumenda! Totam, explicabo. Debitis nulla quam fugiat eaque?
                Corporis error voluptates ipsa nam impedit eius itaque quaerat quibusdam cumque velit sed repudiandae molestias quis sit hic ipsam, est amet quae assumenda officia?<strong>Perspiciatis rerum est dolore</strong> ratione magni?
                Dolores commodi dolore, doloribus iusto fugiat aut nobis laboriosam similique odit esse sapiente, aliquid neque? Excepturi est voluptatem <strong>exercitationem</strong> ratione impedit in, rem fugit vitae quia quaerat illum laborum itaque.</p>
            </div>

           <a href="./alternate-services.php#tuina" class="animation-element animation--style_fade-up">Zu meinem Tuina Angebot</a>
        </article>

        <!-- MOXIBUSTION -->
        <article class="alternate-ways_tcm_topic" id="moxibustion">
            
            <header>
                <h4 class="animation-element animation--style_flipping-headline animation--delay_300">
                    <span class="heading-markup">Moxibustion</span>
                </h4>
        
                <picture class="animation-element animation--style_opacity-in animation--duration_1000">
                    <source srcset="./../graphics/pics/moxibustion-wmc-750.webp" media="(max-width: 750px)">
                    <source srcset="./../graphics/pics/moxibustion-wmc-1500.webp" media="(max-width: 1500px)">
                    <source srcset="./../graphics/pics/moxibustion.webp" media="(min-width: 1500px)">
                    <img src="./../graphics/pics/moxibustion.webp" alt="moxibustion" loading="lazy">
                </picture>
            </header>
            
            <div class="container-description">

                <div class="on-the-point animation-element animation--style_clippy-circle">
                    <p><strong>Lorem ipsum dolor sit, amet consectetur</strong> adipisicing elit. Impedit, esse minus aliquid quaerat molestiae beatae dolores. Eius id consequuntur, nesciunt temporibus, praesentium cum itaque <strong>voluptas saepe</strong> maxime libero architecto voluptatibus.</p>
                </div>

                <p class="animation-element animation--style_clippy-circle animation--delay_200">Lorem ipsum dolor sit amet consectetur adipisicing elit. Necessitatibus praesentium, numquam velit corrupti id, accusantium iusto, nulla <strong>repellat cumque voluptas eveniet</strong> sexcepturi assumenda! Totam, explicabo. Debitis nulla quam fugiat eaque?
                Corporis error voluptates ipsa nam impedit eius itaque quaerat quibusdam cumque velit sed repudiandae molestias quis sit hic ipsam, est amet quae assumenda officia?<strong>Perspiciatis rerum est dolore</strong> ratione magni?
                Dolores commodi dolore, doloribus iusto fugiat aut nobis laboriosam similique odit esse sapiente, aliquid neque? Excepturi est voluptatem <strong>exercitationem</strong> ratione impedit in, rem fugit vitae quia quaerat illum laborum itaque.</p>
            </div>

           <a href="./alternate-services.php#tuina" class="animation-element animation--style_fade-up">Zu meinem Moxibustion Angebot</a>
        </article>

        <!-- NUTRITIONAL ADVICE -->
        <article class="alternate-ways_tcm_topic" id="nutritional-advice">

            <header>
                <h4 class="animation-element animation--style_flipping-headline animation--delay_300">
                    <span class="heading-markup">Ernährungsberatung</span>
                </h4>
            
                <picture class="animation-element animation--style_opacity-in animation--duration_1000">
                    <source srcset="./../graphics/pics/nutritional-wmc-750.webp" media="(max-width: 750px)">
                    <source srcset="./../graphics/pics/nutritional-wmc-1500.webp" media="(max-width: 1500px)">
                    <source srcset="./../graphics/pics/nutritional.webp" media="(min-width: 1500px)">
                    <img src="./../graphics/pics/nutritional.webp" alt="nutritional" loading="lazy">
                </picture>
            </header>
            
            <div class="container-description">

                <div class="on-the-point animation-element animation--style_clippy-circle">
                    <p><strong>Lorem ipsum dolor sit, amet consectetur</strong> adipisicing elit. Impedit, esse minus aliquid quaerat molestiae beatae dolores. Eius id consequuntur, nesciunt temporibus, praesentium cum itaque <strong>voluptas saepe</strong> maxime libero architecto voluptatibus.</p>
                </div>

                <p class="animation-element animation--style_clippy-circle animation--delay_200">Lorem ipsum dolor sit amet consectetur adipisicing elit. Necessitatibus praesentium, numquam velit corrupti id, accusantium iusto, nulla <strong>repellat cumque voluptas eveniet</strong> sexcepturi assumenda! Totam, explicabo. Debitis nulla quam fugiat eaque?
                Corporis error voluptates ipsa nam impedit eius itaque quaerat quibusdam cumque velit sed repudiandae molestias quis sit hic ipsam, est amet quae assumenda officia?<strong>Perspiciatis rerum est dolore</strong> ratione magni?
                Dolores commodi dolore, doloribus iusto fugiat aut nobis laboriosam similique odit esse sapiente, aliquid neque? Excepturi est voluptatem <strong>exercitationem</strong> ratione impedit in, rem fugit vitae quia quaerat illum laborum itaque.</p>
            </div>

           <a href="./alternate-services.php#nutritional-advice" class="animation-element animation--style_fade-up">Zu meinem Ernährungsberatungs Angebot</a>
        </article>

        <!-- QI GONG -->
        <article class="alternate-ways_tcm_topic" id="qi-gong">

            <header>
                <h4 class="animation-element animation--style_flipping-headline animation--delay_300">
                    <span class="heading-markup">Qi Gong</span>
                </h4>

                <picture class="animation-element animation--style_opacity-in animation--duration_1000">
                    <source srcset="./../graphics/pics/qi-gong-wmc-750.webp" media="(max-width: 750px)">
                    <source srcset="./../graphics/pics/qi-gong-wmc-1500.webp" media="(max-width: 1500px)">
                    <source srcset="./../graphics/pics/qi-gong.webp" media="(min-width: 1500px)">
                    <img src="./../graphics/pics/qi-gong.webp" alt="qi-gong" loading="lazy">
                </picture>
            </header>
            
            <div class="container-description">

                <div class="on-the-point animation-element animation--style_clippy-circle">
                    <p><strong>Lorem ipsum dolor sit, amet consectetur</strong> adipisicing elit. Impedit, esse minus aliquid quaerat molestiae beatae dolores. Eius id consequuntur, nesciunt temporibus, praesentium cum itaque <strong>voluptas saepe</strong> maxime libero architecto voluptatibus.</p>
                </div>

                <p class="animation-element animation--style_clippy-circle animation--delay_200">Lorem ipsum dolor sit amet consectetur adipisicing elit. Necessitatibus praesentium, numquam velit corrupti id, accusantium iusto, nulla <strong>repellat cumque voluptas eveniet</strong> sexcepturi assumenda! Totam, explicabo. Debitis nulla quam fugiat eaque?
                Corporis error voluptates ipsa nam impedit eius itaque quaerat quibusdam cumque velit sed repudiandae molestias quis sit hic ipsam, est amet quae assumenda officia?<strong>Perspiciatis rerum est dolore</strong> ratione magni?
                Dolores commodi dolore, doloribus iusto fugiat aut nobis laboriosam similique odit esse sapiente, aliquid neque? Excepturi est voluptatem <strong>exercitationem</strong> ratione impedit in, rem fugit vitae quia quaerat illum laborum itaque.</p>
            </div>

           <a href="./alternate-services.php#qi-gong" class="animation-element animation--style_fade-up">Zu meinem Qi Gong Angebot</a>
        </article>
    </section>

    <!-- SCHRÖPFEN -->
    <section class="alternate-ways_cupping" id="cupping">
        <div class="alternate-ways_cupping_left-side">
            <picture class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000">
                <source srcset="./../graphics/pics/cupping-wmc-750.webp" media="(max-width: 750px)">
                <source srcset="./../graphics/pics/cupping-wmc-1500.webp" media="(max-width: 1500px)">
                <source srcset="./../graphics/pics/cupping.webp" media="(min-width: 1500px)">
                <img src="./../graphics/pics/cupping.webp" alt="cupping" loading="lazy">
            </picture>
        </div>

        <div class="alternate-ways_cupping_right-side">
            <h3 class="animation-element animation--style_fade-up">Schröpfen</h3>

            <p class="on-the-point animation-element animation--style_clippy-left-top-slide animation--delay_200">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Ratione deleniti iusto minima corporis distinctio, veritatis doloremque! Quidem, in. Aspernatur illum atque inventore cumque nulla consequuntur assumenda mollitia architecto quo corporis.Quia velit eum exercitationem accusamus ea iste vero dignissimos molestiae aliquam dolorem ad, nemo aliquid, fuga modi et quos iure voluptates perspiciatis aspernatur impedit commodi. Quia pariatur velit officia corporis.
            </p>

            <p class="animation-element animation--style_fade-from-right">Lorem ipsum, dolor sit amet consectetur adipisicing elit. Voluptatem officiis, deleniti natus similique delectus cum quibusdam cumque, corporis recusandae doloribus eveniet dolorum, vero vitae eum mollitia. Tempora autem ad totam.
            Veniam cupiditate exercitationem quis, eligendi ut tempore doloribus possimus, facere, expedita debitis impedit veritatis voluptate perferendis ullam! Dolorum, nihil perferendis, minus praesentium earum dolor, impedit non possimus expedita animi asperiores! >Lorem ipsum dolor sit amet consectetur adipisicing elit. Aut ab labore a enim, veniam, dicta doloremque tempora odit nesciunt repudiandae reiciendis possimus, molestiae placeat consectetur amet excepturi recusandae ipsam laboriosam?</p>

            <a href="./alternate-services.php#cupping" class="animation-element animation--style_fade-up">Zu meinem Schröpf Angebot</a>
        </div>
    </section>

    <!-- TIBETIAN MASSAGE -->
    <section class="alternate-ways_tibetian-massage" id="tibetian-massage">
        <div class="alternate-ways_tibetian-massage_left-side">

            <picture class="animation-element animation--style_clippy-from-right-to-left animation--duration_1000">
                <source srcset="./../graphics/pics/tibetian-massage-wmc-750.webp" media="(max-width: 750px)">
                <source srcset="./../graphics/pics/tibetian-massage-wmc-1500.webp" media="(max-width: 1500px)">
                <source srcset="./../graphics/pics/tibetian-massage.webp" media="(min-width: 1500px)">
                <img src="./../graphics/pics/tibetian-massage.webp" alt="tibetian-massage" loading="lazy">
            </picture>
        </div>

        <div class="alternate-ways_tibetian-massage_right-side">
            <h3 class="animation-element animation--style_fade-up">Tibetanische Massage</h3>

            <p class="on-the-point animation-element animation--style_clippy-right-top-slide animation--delay_200">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Ratione deleniti iusto minima corporis distinctio, veritatis doloremque! Quidem, in. Aspernatur illum atque inventore cumque nulla consequuntur assumenda mollitia architecto quo corporis.Quia velit eum exercitationem accusamus ea iste vero dignissimos molestiae aliquam dolorem ad, nemo aliquid, fuga modi et quos iure voluptates perspiciatis aspernatur impedit commodi. Quia pariatur velit officia corporis.
            </p>

            <p class="animation-element animation--style_fade-from-left">Lorem ipsum, dolor sit amet consectetur adipisicing elit. Voluptatem officiis, deleniti natus similique delectus cum quibusdam cumque, corporis recusandae doloribus eveniet dolorum, vero vitae eum mollitia. Tempora autem ad totam.
            Veniam cupiditate exercitationem quis, eligendi ut tempore doloribus possimus, facere, expedita debitis impedit veritatis voluptate perferendis ullam! Dolorum, nihil perferendis, minus praesentium earum dolor, impedit non possimus expedita animi asperiores! >Lorem ipsum dolor sit amet consectetur adipisicing elit. Aut ab labore a enim, veniam, dicta doloremque tempora odit nesciunt repudiandae reiciendis possimus, molestiae placeat consectetur amet excepturi recusandae ipsam laboriosam?</p>

            <a href="./alternate-services.php#tibetian-massage" class="animation-element animation--style_fade-up">Zu meinem Tibetanische Massage Angebot</a>
        </div>
    </section>

    <!-- QI GONG GROUP -->
    <section class="alternate-ways_qi-gong-group" id="qi-gong-group">
        <header class="alternate-ways_qi-gong-group_header">
            <picture class="animation-element animation--style_clippy-from-top-to-bottom animation--duration_1000">
                <source srcset="./../graphics/pics/qi-gong-group-wmc-750.webp" media="(max-width: 750px)">
                <source srcset="./../graphics/pics/qi-gong-group-wmc-1500.webp" media="(max-width: 1500px)">
                <source srcset="./../graphics/pics/qi-gong-group.webp" media="(min-width: 1500px)">
                <img src="./../graphics/pics/qi-gong-group.webp" alt="qi-gong-group" loading="lazy">
            </picture>
            <h3 class="animation-element animation--style_clippy-circle">Qi Gong Gruppe</h3>
        </header>

        <div class="alternate-ways_qi-gong-group_description">
            <p class="animation-element animation--style_fade-from-left">Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla enim accusamus at quae, vero adipisci illo cumque tempora, eligendi omnis minus optio delectus commodi neque deserunt ipsam alias earum magnam!Aut dolores asperiores harum sit at fugiat recusandae commodi veritatis numquam itaque unde nemo, magnam nam explicabo, consequuntur quaerat architecto. Quas, sapiente. Eaque amet hic labore mollitia deleniti voluptate architecto.</p>

            <dl>
                <dt class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000"><span>Wann</span></dt>
                <dd class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000 animation--delay_400">Montags und Mittwochs von 18:30 - 20:00</dd>

                <dt class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000"><span>Wo</span></dt>
                <dd class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000 animation--delay_400">Schloßweg 5, 9020 Klagenfurt</dd>

                <dt class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000"><span>Teilnehmeranzahl</span></dt>
                <dd class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000 animation--delay_400">12</dd>

                <dt class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000"><span>Kosten</span></dt>
                <dd class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000 animation--delay_400">Freiwillige Spende</dd>
            </dl>
        </div>

        <div class="alternate-ways_qi-gong-group_request">
            <p class="animation-element animation--style_opacity-in">
                Bei Interesse kontaktieren Sie mich wahlweise per Mail oder Telefon :
            </p>

            <a class="mail-link animation-element animation--style_fade-up animation--delay_200" href="mailto:office@dr-windblade.com?subject=Interesse%20an%20der%20Meditationsgruppe&body=Guten%20Tag%2C%20%0Aich%20w%C3%BCrde%20gerne%20an%20der%20Meditationsgruppe%20teilnehmen.">
                <img src="../graphics/icons_n_co/mail.svg" alt="mail">
                <span>office@dr-windblade.com</span>
            </a>

            <a href="tel:+4365055566643" class="animation-element animation--style_fade-up animation--delay_400">
                <img src="../graphics/icons_n_co/phone.svg" alt="phone">
                <span>0650/55566643</span>
            </a>
        </div>
    </section>

    <!-- MEDITATION GROUP -->
    <section class="alternate-ways_meditation-group" id="meditation-group">
        <header class="alternate-ways_meditation-group_header">
            <picture class="animation-element animation--style_clippy-from-top-to-bottom animation--duration_1000">
                <source srcset="./../graphics/pics/meditation-group-wmc-750.webp" media="(max-width: 750px)">
                <source srcset="./../graphics/pics/meditation-group-wmc-1500.webp" media="(max-width: 1500px)">
                <source srcset="./../graphics/pics/meditation-group.webp" media="(min-width: 1500px)">
                <img src="./../graphics/pics/meditation-group.webp" alt="meditation-group" loading="lazy">
            </picture>
            <h3 class="animation-element animation--style_clippy-circle">Meditations Gruppe</h3>
        </header>

        <div class="alternate-ways_meditation-group_description">
            <p class="animation-element animation--style_fade-from-left">Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla enim accusamus at quae, vero adipisci illo cumque tempora, eligendi omnis minus optio delectus commodi neque deserunt ipsam alias earum magnam!Aut dolores asperiores harum sit at fugiat recusandae commodi veritatis numquam itaque unde nemo, magnam nam explicabo, consequuntur quaerat architecto. Quas, sapiente. Eaque amet hic labore mollitia deleniti voluptate architecto.</p>

            <dl>
                <dt class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000"><span>Wann</span></dt>
                <dd class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000 animation--delay_400">Dienstags und Donnerstags von 18:30 - 20:00</dd>

                <dt class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000"><span>Wo</span></dt>
                <dd class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000 animation--delay_400">Waldweg 7, 9020 Klagenfurt</dd>

                <dt class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000"><span>Teilnehmeranzahl</span></dt>
                <dd class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000 animation--delay_400">12</dd>

                <dt class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000"><span>Kosten</span></dt>
                <dd class="animation-element animation--style_clippy-from-left-to-right animation--duration_1000 animation--delay_400">Freiwillige Spende</dd>
            </dl>
        </div>

        <div class="alternate-ways_meditation-group_request">
            <p class="animation-element animation--style_opacity-in">
                Bei Interesse kontaktieren Sie mich wahlweise per Mail oder Telefon :
            </p>

            <a class="mail-link animation-element animation--style_fade-up animation--delay_200"  href="mailto:office@dr-windblade.com?subject=Interesse%20an%20der%20Meditationsgruppe&body=Guten%20Tag%2C%20%0Aich%20w%C3%BCrde%20gerne%20an%20der%20Meditationsgruppe%20teilnehmen.">
                <img src="../graphics/icons_n_co/mail.svg" alt="mail">
                <span>office@dr-windblade.com</span>
            </a>

            <a href="tel:+4365055566643" class="animation-element animation--style_fade-up animation--delay_400">
                <img src="../graphics/icons_n_co/phone.svg" alt="phone">
                <span>0650/55566643</span>
            </a>
        </div>
    </section>

</section>


<?php require __DIR__ . './../views/footer.php'; ?>